...
| # | 확인 | 결과 |
|---|---|---|
| 1 | Pull 후 Safe Mode·컴파일 오류 없이 열리고 새 변경점이 생기지 않는다 | 미확인 |
| 2 | `Intro` Play → 하늘색 타이틀 `CHESS FIGHT`, "6 vs 6 · 체스 말 파티 게임", 아무 키 → 로비 | 미확인 |
| 3 | 로비: 하늘·흰 바닥·금색 원 위에 **내 캐릭터가 가운데**, 머리 위에 `★ 내 이름 / 파티장 · 1/6`, 양옆에 회색 실루엣과 노란 `+`·`친구 초대` | 미확인 |
| 4 | 상단 바(로고, 플레이 탭, `친구 n`, `정보`, 내 이름), 좌측 하단 파티 바(PARTY 1/6, 파티 코드, AI 봇 - 0 +, 파티 나가기), 우측 하단 모드 카드 `킹 러시` + 노란 `게임 시작 Enter`가 보이고 **한글이 깨지지 않는다** | 미확인 |
