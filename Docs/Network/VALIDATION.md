...
최종 갱신: 2026-09-25 (KST). 코드 기준: `JY-lobby` 브랜치(로비·게임모드), 그 이전 항목은 `Network`/`main`. 전체 상황은 루트 [`HANDOFF.md`](../../HANDOFF.md).
대상: Unity `6000.3.11f1`, Windows x64, Steamworks.NET `2025.164.1`.

상태: **자동 검사 통과(Core 29, 세션 15, 참조 컴파일) / 새 로비·게임 모드(09-25, 프로토콜 v3)는 Unity 미확인 / 로비 HUD·한글·클릭·친구 초대 Unity 확인(09-24) / 두 PC 파티 입장·공개 매칭 성사 확인(09-22) / 두 계정 이동 동기화 보고 없음 / 씬 분리(72ddf9e)·연결 품질(01dd655)·래그돌 병합(09-25)은 Unity 미확인**.

**새 확인 항목은 이 아래 ★ 표처럼 맨 위에 추가한다.** 사용자가 확인하면 '결과' 칸에 날짜와 함께 적는다.

## ★ 2026-09-26 퀸 오브 더 힐 래그돌 기능 M1~M4 (`JY-ragdoll_v2`) — 확인 목록

탈것·입력 확장·피격·물 부활을 만들었다([Player/RAGDOLL §8](../Player/RAGDOLL.md#8-퀸-오브-더-힐-기능-m1m4)). **빌드한 랩 플레이어의 자동 점검은 새 항목 12개가 모두 통과했지만, Unity 화면에서 사람이 본 것은 아직 없다.** 랩 온라인은 **프로토콜 v4**라 두 PC 모두 이 브랜치여야 한다.

| # | 확인 (`ChessFight > Scenes > Ragdoll Test` → Play, 화면 한 번 클릭) | 결과 |
|---|---|---|
| 1 | Pull 후 컴파일 오류 없이 열린다. Play 중 Console에 빨간 오류가 없다 | 미확인 |
| 2 | 화면 왼쪽 아래에 **[7] 퀸 오브 더 힐 시험대** 창이 보이고, E·Q를 누르면 "능력 n회"가 1씩 오르고, F를 누르고 있으면 "(누르는 중)", 마우스로 시점을 돌리면 조준 숫자가 바뀐다 | 미확인 |
| 3 | **F4**로 자유 카메라가 켜지고 꺼진다(F가 아니다) | 미확인 |
| 4 | **F9** → 남동쪽 시험대 입구로 이동. 바닥에 `[7a]`~`[7e]` 글씨가 보인다(한글이 네모로 나오면 알려 주기) | 미확인 |
| 5 | [7a] 노란 왕복 발판에 점프해 올라타 10초 가만히 → 미끄러지거나 떨어지지 않는다. 발판이 달릴 때 Space → 다시 발판 위에 내린다 | 미확인 |
| 6 | [7b] 승강기에 올라타 12 m 꼭대기까지 → 떨어지지 않는다. 올라가는 중 Space → 다시 승강기에 내린다. 꼭대기에서 옆 탑으로 건너갈 수 있다 | 미확인 |
| 7 | [7c] 회전 원판 가장자리 쪽에 서 있기 → 원판과 같이 돌고(몸 방향도), 밀려나지 않는다 | 미확인 |
| 8 | [7d] 움직이는 벽에 W + 우클릭으로 매달려 우클릭만 누르고 있기 → 벽을 따라 옆으로 실려 가고 떨어지지 않는다 | 미확인 |
| 9 | 평지에서 **F5** → 뒤로 날아가 약 1초 누워 있다가 일어난다 | 미확인 |
| 10 | 벽에 매달린 채 **F6** → 머리 옆 스테미나 게이지가 약 30% 줄고 계속 매달려 있다. **F7** → 곧바로 벽에서 떨어진다 | 미확인 |
| 11 | [7e] 부두 끝에서 기둥에 W + 우클릭으로 매달려 → A 또는 D로 옆으로 → S로 물까지 내려간다 → 약 2초 뒤 초록 체크포인트에 **서서** 나온다(벽에 붙어 있지 않다) | 미확인 |
| 12 | [7e] 상자를 우클릭으로 잡고 물에 뛰어든다 → 약 2초 뒤 체크포인트에 서서 나오고, 상자를 쥐고 있지 않으며, **다시 물로 끌려가지 않는다** | 미확인 |
| 13 | (선택) 두 PC 랩 온라인(F3): 참가자가 시험대에서 탈것을 타면 양쪽 화면에서 같게 보인다 | 미확인 |
