...
2. PC A: 좌측 `비공개 방 만들기` → 우측 상단 `경기 복사`.
3. PC B: `붙여넣기` → `경기 참가`.
4. 명단 2명, 파랑·주황 캡슐. **WASD 이동, Space 점프.** 입력칸 밖을 클릭해야 이동한다.
5. A가 움직일 때 B 화면에서도 움직이는지, 반대도 확인한다. ← **아직 보고된 적 없는 가장 중요한 확인**
6. A: `경기 시작`(비공개 2명 이상) → 둘 다 KingRush로 넘어가는지. Esc → 로비 복귀, 파티 유지.
7. 연결 품질: 상세 정보·경기 화면의 `핑 / 응답`, B에서 F8을 눌러 지연을 켜고 점프가 빠지지 않는지, A 창을 일시정지해 B에 노랑 → 빨강 배너가 뜨는지.

같은 팀으로 시험하려면 먼저 `파티 복사` → 상대 `파티 참가`(또는 친구 초대)로 같은 파티를 만든 뒤 파티장이 방을 만든다.

## 3. 친구와 파티 + 자동 매칭

- 실행하면 각자 1인 파티가 생긴다.
- `게임 시작` → `파티 생성` → `친구 초대`: 게임 안 친구 목록에서 `초대`. 받는 쪽은 Steam 초대를 수락한다. 또는 Steam 친구 목록의 "게임 참가"(Rich Presence).
- 파티장이 `게임 시작`(파티 화면) 또는 `빠른 매칭`을 누르면 파티 전체가 매칭된다. 파티원은 자동으로 따라간다.
