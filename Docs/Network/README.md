...
1. PC A: 좌측 `비공개 방 만들기` → 우측 상단 `경기 복사`로 방 번호 복사.
2. PC B: 그 번호를 좌측 `번호` 칸에 붙여넣고 `경기 참가`.
3. 두 플레이어가 입장 승인되면 파랑/주황 캡슐이 생성된다. **WASD 이동, Space 점프**. 번호 입력칸 밖을 클릭해야 이동 입력이 작동한다.
4. A에서 움직일 때 B에도 같은 이동이 보이는지, 반대 방향도 확인한다.
5. 우측 하단 `경기 시작`은 비공개 방 2명 이상, 공개 방 12명일 때만 활성화된다. 누르면 세션을 시작하고 추가 입장을 닫는다. 시작 전 대기 공간에서도 움직일 수 있다.
6. B에서 `취소` → A에서 B의 캡슐이 사라지는지 확인한다.

각 PC의 독립 파티가 같은 테스트 방에 들어오는 방식이다. 두 사람이 같은 팀이어야 한다면 먼저 아래의 **파티 ID**로 함께 파티를 만든 뒤 방을 생성한다.

## 화면 구성

HUD는 화면 전체를 쓴다. 문구는 모두 한국어다.

| 위치 | 내용 |
|---|---|
| 좌측 상단 | 상태·오류, Steam 다시 연결 |
| 좌측 중단 | AI 봇 조절, 번호로 직접 입장, WASD 토글 |
| 좌측 하단 | 내 파티 / 경기 명단 |
| 우측 상단 | 파티·경기 번호와 복사 버튼 |
| **우측 하단** | **큰 액션 버튼** |

액션 버튼은 단계별로 바뀐다. **`게임 시작`을 눌러도 바로 시작되지 않는다.**

```text
게임 시작  →  [파티 생성]  →  [친구 초대] [게임 시작] [파티 나가기]
           →  [빠른 매칭]  →  바로 매칭
```

매칭이 돌거나 경기 방에 들어가면 자동으로 `경기 시작` / `취소`로 바뀐다.
...
- **PC 2대로 12인 테스트:** 양쪽이 각각 1인 파티 + 봇 5로 `게임 시작 → 빠른 매칭` → 6 대 6이 채워져 자동 시작한다.

**방 채우기 (`12명 채우기`)**

- 경기 호스트만, 시작 전에만 쓸 수 있다. 빈 자리를 봇으로 한 번에 채운다.
- **PC 1대로 12인 테스트:** `비공개 방 만들기` → 파티 봇 5 → `12명 채우기` → 12인. 호스트 CPU와 대역폭을 혼자 측정할 수 있다.
- `봇 비우기`로 되돌린다. 공개 방에서 쓰면 남은 자리가 사라져 실제 플레이어가 못 들어온다.

봇은 정해진 지점으로 걸어가고 가끔 점프하는 수준이다. **게임 AI가 아니라 인원·부하 채우기용이다.** 킹러시 규칙이나 기물 스킬은 들어 있지 않다.

## 연결 품질과 버전 (2026-09-24)

- **끊김 경고:** 방장 스냅샷이 0.5초 없으면 노란 "연결 불안정", 2초면 빨간 "멈춤"(내 캐릭터도 멈춤), 12초면 파티로 돌아온다.
- **핑:** 로비 상세 정보와 킹러시 왼쪽 카드에 `핑`(Steam 전송)과 `응답`(입력이 방장에게 반영되어 돌아오기까지)을 표시한다. 방장은 가장 나쁜 참가자의 핑을 본다.
- **F8 지연 시뮬레이터 (Editor·개발 빌드):** 꺼짐 → +100ms → +200ms/손실 5% → +300ms/손실 10%. 누른 PC의 송·수신에만 적용된다.
- **버전:** 로비에 `build`(Player Settings Version + 개발 빌드면 `-dev`)를 기록한다. 다른 버전끼리는 매칭되지 않고, 초대로 들어가면 "게임 버전이 다릅니다"가 뜬다. **테스터에게 빌드를 줄 때마다 Version을 올린다.**
- **봇 규칙:** 릴리스 빌드는 공개 매칭에 봇을 넣을 수 없다(비공개 테스트 방은 가능). Editor·개발 빌드는 지금처럼 쓸 수 있다.
- **Steam 친구 목록 참가:** 파티가 대기 중이면 친구가 Steam 친구 목록에서 "게임 참가"로 바로 들어올 수 있다.

## 친구 파티 + 자동 매칭

- 실행 시 각자 1인 비공개 파티가 만들어진다. 파티 최대 6명.
- `친구 초대`: **게임 안에서 친구 목록이 뜬다.** 게임 중 → 온라인 → 자리 비움 → 오프라인 순으로 정렬되며, 각 친구의 `초대` 버튼이 오버레이 없이 바로 초대를 보낸다. `온라인만`/`전체 보기` 전환과 페이지 이동(◀ ▶)이 있다.
- 친구 패널이 안 되면 `Steam 오버레이로 초대` 버튼이 기존 방식으로 열어준다. 그것도 안 되면 `파티 복사` → 친구의 `붙여넣기` → `파티 참가`.
- 매칭 중에는 파티가 잠기므로 초대할 수 없다. 패널이 자동으로 닫힌다.
- **파티 번호**는 친구 그룹, **경기 번호**는 12인 경기 방이다. 서로 다른 버튼으로 입장한다.
- 파티장만 `게임 시작`(우측 하단) → `빠른 매칭`을 누른다. 파티원은 파티장의 매칭 결과를 자동으로 따라간다.
