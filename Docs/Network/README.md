...
2. Unity를 열면 `NetworkSetup`이 **Steamworks.NET 2025.164.1**을 Package Manager로 설치한다. Git 커밋 `c21a8f0e31c56ae8707130967faf491f7dd7c0d8`에 고정했다.
...
4. 설치에 성공하면 Unity가 변경한 **`Packages/manifest.json`, `Packages/packages-lock.json`을 함께 커밋**한다. 이후 팀원은 고정된 의존성을 자동으로 복원한다. 설치 도구는 이미 설치된 패키지를 반복 추가하지 않는다.
...
첫 Pull은 패키지 설치를 시작하는 부트스트랩을 포함한다. 이 커밋의 manifest/lock에는 아직 Steamworks가 없다. 작업 환경의 Unity 라이선스 연결이 막혀 UPM 설치 결과를 여기서 생성하지 못했기 때문이다. 패키지가 설치될 때까지 Steam 런타임 어셈블리는 조건부로 제외된다.
...
- **실제 Steam 2계정 접속, 빌드, UI 표시, 인터넷 지연·패킷 손실은 아직 실기 검증이 필요하다.** 현재 결과는 출시 완료 판정이 아니다.
