...
함께 첨부하면 좋은 것: **사용자의 공식 아트 컨셉 이미지 두 장**(기물 6종 라인업, 킹 러시 한 장 요약)과 **탑 배치 스케치**(가운데 탑, 아래 물, 양쪽 출발 다리) — 저장소에는 없다(DESIGN §3.8). **중앙 탑은 컨셉 이미지를 따르지 않는다**(2차 탑 확대, R29). 그리고 9/19 기획개발 핸드오프, 기본 기획 v0.2 PDF, 전체 기획안 v0.3(§3.3). 로비 참고 영상은 로비 작업용이라 필요 없다.
