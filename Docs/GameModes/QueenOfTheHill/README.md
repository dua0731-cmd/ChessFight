...
작성 2026-09-25, 브랜치 `JY-lobby`. **2026-09-26 추가:** 래그돌 쪽 새 기능(M1 탈것·M2 입력·M3 피격·M4 물)은 사용자가 만든 **`JY-ragdoll_v2`**에서 구현됐다(R34) → [MECHANICS_TODO "진행 상황"](MECHANICS_TODO.md#진행-상황-2026-09-26-jy-ragdoll_v2), [Player/RAGDOLL §8](../../Player/RAGDOLL.md#8-퀸-오브-더-힐-기능-m1m4). 아래의 "`JY-lobby`에서는 래그돌을 바꾸지 않는다" 규칙은 그대로다. 이 모드를 **구체적인 기획부터 개발까지** 다른 AI(ChatGPT 아스트라)가 이어받기 위한 문서다. 사용자가 요청했다(R24).
...
§7 '남은 결정'(§5 게임 흐름·§6 기물 밸런스 제안의 확정, 아군 밀치기, UI·연출)을 나와 하나씩 정해
DESIGN.md를 확정본으로 만들어 줘. 그다음 README §5 순서(A→B→C→D)로 개발한다.
```

함께 첨부하면 좋은 것: **사용자의 공식 아트 컨셉 이미지 두 장**(기물 6종 라인업, 킹 러시 한 장 요약)과 **탑 배치 스케치**(가운데 탑, 아래 물, 양쪽 출발 다리) — 저장소에는 없다(DESIGN §3.8). **중앙 탑은 컨셉 이미지를 따르지 않는다.** 레벨 구조는 5차로 승인(R31), 아트는 사용자가 고른 이미지 A 베이스의 "하늘 궁전"(R32, [ART_CONCEPTS §5](ART_CONCEPTS.md)). **팀·캐릭터 색은 흑·백**이라 맵에는 흑백을 쓰지 않는다. 그리고 9/19 기획개발 핸드오프, 기본 기획 v0.2 PDF, 전체 기획안 v0.3(§3.3). 로비 참고 영상은 로비 작업용이라 필요 없다.
