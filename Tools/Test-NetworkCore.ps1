...
$sources = Get-ChildItem (Join-Path $projectRoot 'Assets/Scripts/Core') -Filter '*.cs' | ForEach-Object FullName
...
$sessionSources = Get-ChildItem (Join-Path $projectRoot 'Assets/Scripts/Core') -Filter '*.cs' | ForEach-Object FullName
$sessionSources += Join-Path $projectRoot 'Assets/Scripts/Network/SteamSession.cs'
