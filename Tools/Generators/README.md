...
| `gen_scenes.py` | 2026-09-24 Intro·KingRush·RagdollTest 씬, 장애물·캐릭터 프리팹, 코스 재질을 Unity YAML로 생성 | **덮어쓴다.** `--overwrite` 없이는 실행되지 않는다. Unity나 사람이 한 번이라도 저장한 뒤에는 돌리지 않는다. 현재 커밋의 파일과 바이트 단위로 같은 결과를 낸다(2026-09-24 확인) |
